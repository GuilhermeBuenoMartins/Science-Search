#Processamento de Linguagem Natural com StopWords
#Grafico da Lei de Zipf e palavras
...
filtro = set(['\'','\"','\\','/','?','!', '@','#','$','%','&','*','(',')','-','_','+','{','}','[',']','{','}','ª','º','´','`','~','^','<','>',';',':','.',',','...','´´','``','\'\'','”']) #Criando um filtro
palavras = [palavra for palavra in palavras if palavra not in filtro]#Removendo as pontuações

#Tirando repetições de palavras
contagem=[]#Frequência das palavras
p_filtradas=[]#palavras sem repetições
...
for i in range(len(palavras)):
    ...
    if palavra not in p_filtradas:#Verifica se a palavra já existe me p_filtradas
        ...
        contagem[p_filtradas.index(palavra)]+=1
...
plt.title('Processamento de Linguagem Natural com StopWords')
